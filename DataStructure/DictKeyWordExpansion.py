...
def circ_area(**kw):    # The parameter is MISSING.  Add it back. 
    ...
    #assert r is not None and assert d is not None
    if 'radius' in kw and 'diameter' in kw:
        assert False, "Cannot specify both radius and diameter"
    
    if 'radius' not in kw and 'diameter' not in kw:
        assert False, "Either radius or diameter need to be specified"

    if 'diameter' in kw:
        r = kw['diameter']/2
        area = math.pi * (r **2)   
    elif 'radius' in kw:
        area = math.pi * (kw['radius'] **2) 
    else:   
        assert 'radius' is None, 'Not enough information given to determine area'
