...
def reports_above_temp(weather,temp):
    ...
    for dt in weather:
        # Check if this report has a temperature measurement
        if "temperature" in weather[dt]:
            mtemp = weather[dt]["temperature"]["value"]
            unit = weather[dt]["temperature"]["units"]
            
            # Convert to Celsius if needed
            if unit == "F":
                mtemp = to_celsius(mtemp)
